...
The various tools described in Section~\ref{cmdTools} are each an experiment in user interaction. As this is an entirely novel user interface, it is valuable to consider each of these tools with respect to their usability and utility to a modeller.

\begin{itemize}
\item{\textbf{key.handle}: }

\item{\textbf{edgelist.dump}, \textbf{pointlist.dump}, \textbf{polylist.dump}: }

\item{\textbf{text.draw}: }

\item{\textbf{vertex.create}: }

\item{\textbf{vertex.remove}: }

\item{\textbf{vertex.move}: }
...
\item{\textbf{code.save}, \textbf{code.load}: }
