...
global proc launchViewer() {
    ...
    // Invoke tool to load file
    $pipe = popen( "/path/to/Modeller", "w" );
    if ( $pipe != 0 ) {
        fprint( $pipe, "obj.load "+$file );
        fclose( $pipe );
    }
}

global proc loadFromViewer() {
    $file = "/path/to/Modeller/scanner_model.obj";
    
    for( $del in `ls -l -type "mesh"` ) {
        if ( objExists( $del ) )
            delete $del;
    }  
    
    file -import -type "OBJ" -ra true -namespace "object"  -pr 
            -loadReferenceDepth "all" $file;
}
\end{verbatim}
