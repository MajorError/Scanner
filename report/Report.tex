...
Therefore, with arbitrary vectors $A$ and $B$, $A ~<~ B~$ iff \\$~(A - O ~x~ A - V) ~~<~~ (B - O ~x~ B - V)$. \\
}
...
$ \left[ 
  \begin{array}{c}
    lock_x * (C'_x + z'_1 * Projection_x) + lerp_x \\ 
    lock_y * (C'_y + z'_2 * Projection_y) + lerp_y \\ 
    lock_z * (C'_z + z'_3 * Projection_z) + lerp_z
  \end{array}\right]$
\\
}
