...
\title{\setcounter{page}{0} Real-Time Interactive 3D Modelling with Commodity Hardware
\vspace{70pt}}
...
One common system used in the visual effects industry is LIDAR capture \cite**{lidar}, in which a scanning laser is used to trace the distance between it and the target object. The object (or the scanner) is typically rotated during this process in order to generate a dense, 360$^\circ$ point-cloud representation of the scanned object. This technique, while expensive, is applicable in a variety of scenarios; from high-resolution facial capture through to large scale airborne ground topology scanning. This process requires little user intervention, and generates a large amount of data relatively quickly. However, much post-processing of this data is typically required; as it is a slightly noisy point-cloud, it must be cleaned up before it can be used in any real application. Furthermore, this cloud of points is not, by itself, geometry; it is more a set of candidate vertices for geometry. If the consumer is not performing volumetric rendering, they must triangulate this data in 3D, which could potentially result in discarding many of the points because they lie on the surfaces of these triangles.

\section{Automated Vision-Based Reconstruction}
...
\begin{figure}
  ...
  \end{center}
  \caption[An Egyptian Sphinx, viewed as Photosynth's point cloud]{An Egyptian Sphinx, viewed as Photosynth's point cloud \protect \cite**{photosynth}}
...
\begin{figure}
  ...
  \end{center}
  \caption[The stages of ProFORMA's automated modelling process]{The stages of ProFORMA's automated modelling process \protect \cite**{proforma}}
  \label{proformaeg}
\end{figure}
...
\begin{figure}[t]
  ...
  \end{center}
  \caption[An example of point projection from the human face using structured light]{An example of point projection from the human face using structured light \protect \cite**{lightface}}
...
\begin{figure}
  ...
  \end{center}
  \caption[A progression of $\alpha$-shapes, with $\alpha$ ranging from $\infty$ to 0 left-to-right and top-to-bottom]{A progression of $\alpha$-shapes, with $\alpha$ ranging from $\infty$ to 0 left-to-right and top-to-bottom \protect \cite**{alpha}}
  \label{alphashapes}
\end{figure}

The notion of 3-dimensional $\alpha$-shapes \cite**{alpha} were introduced as an early attempt to fit a manifold onto a cloud of points in 3D space, based on Delaunay Triangulation (and their dual, Voronoi diagrams). These shapes are a mathematically rigorous generalisation of the convex hull approach \cite**{hull}, with an additional parameter (the eponymous $\alpha$), which defines the range of the family of shapes; as $\alpha$ tends towards 0, the shape tends towards the point cloud. As $\alpha$ tends towards $\infty$, the shape tends towards the convex hull; for values in between, the shape is somewhere between the point cloud and the convex hull - it is not guaranteed that this $\alpha$-shape is fully connected, or necessarily convex, as the convex hull must be. See Figure~\ref{alphashapes} for an example of this in two disconnected tori.
...
\section{User-Driven Vision-Based Reconstruction}

\begin{figure}
  \begin{center}
    \includegraphics[width=340px]{videotrace}
  \end{center}
  \caption[An example of the VideoTrace User Interface in action]{An example of the VideoTrace User Interface in action \protect \cite**{videotrace}}
  \label{videotraceeg}
\end{figure}
...
\begin{figure}
  ...
  \end{center}
  \caption[A comparison of the various stages of multi-view texture extraction]{A comparison of the various stages of multi-view texture extraction \protect \cite**{texmultiview}}
  \label{texture}
\end{figure}
...
\begin{figure}
  ...
  \subfigure[Key Frame]
  {
    \includegraphics[width=165px]{Debevec1}
  }
  \hspace{-10px}
  \subfigure[Offset Image]
  {
    \includegraphics[width=165px]{Debevec3}
  }
  \subfigure[Warped Offset Image]
  {
    \includegraphics[width=165px]{Debevec2}
  }
  \hspace{-10px}
  \subfigure[Map of Image Disparity]
  {
    \includegraphics[width=165px]{Debevec4}
  }
  \subfigure[Novel View 1]
  {
    \includegraphics[width=165px]{Debevec5}
  }
  \hspace{-10px}
  \subfigure[Novel View 2]
  {
    \includegraphics[width=165px]{Debevec7}
  }
  \caption[Stages of Debevec's approach to View-Dependent Texture Mapping]{Stages of Debevec's approach to View-Dependent Texture Mapping \protect \cite**{architecture}}
  \label{debevecviews}
  \end{center}
\end{figure}

The final key aspect of these approaches we have not studied in detail is the problem of extracting texture from the input video, and mapping that onto the output geometry. There are a number of elements to this; selection of video frames for texture, compensation for lens distortion and projection of the texture into 2D space, and mapping of this texture onto the generated model such that it forms a contiguous view. A popular technique is to use texture from multiple camera views \cite**{texmultiview} in order to minimise distortions at the boundaries; see Figure~\ref{texture} for an illustration of this. The approach outlined by Niem and Broszio requires calibration of the camera \cite**{camcalib} in order to remove distortion and reduce aberrations in the input image due to lens shape. It assigns triangles on the surface of the mesh a camera image (selected according to best resolution). Groups of triangles which are textured by the same image (within some tolerance) are then considered to be \textit{surface regions}. These regions are refined so as to group them together as much as possible, reducing the number of region boundaries. Finally, it filters textures at the boundaries by linearly interpolating the texture from one view to the other across the boundary edge. The result is a smoothly varying texture over the surface of the shape; however, it does necessitate choosing sub-optimal video frames for some polygons in the texture-map. The end result of this is a texture map which requires less space to store, and varies smoothly, and thus the trade-off is probably acceptable. Niem and Broszio also describe a technique for synthesising texture for polygons which lack information, by reflecting texture from adjacent regions and blending the results.
	...
	\begin{tikzpicture}[scale=0.9]
	    ...
	    child[concept color=blue,text=white] {
	      ...
	      child[concept color=red,text=black] {
	        ...
	        child[concept color=orange,text=black,sibling angle=90] { 
...
\begin{figure}
  \begin{center}
	...
	\begin{tikzpicture}[scale=0.75]
	  \begin{class}[text width=150px]{Environment}{0,1.45}
...
\begin{figure}[h]
  \begin{center}
        ...
        \begin{tikzpicture}[scale=0.6]
          ...
          \begin{class}[text width=40px]{PTAM}{0.25,-10}
...
\noindent
\ttfamily
\hlstd{}\hllin{\ 1\ }\hlkwd{MK\textunderscore VISION\textunderscore PLUGIN}\hlstd{}\hlopt{(\ }\hlstd{MyVisionAlgorithm}\hlopt{,\ }\hlstd{pthread\textunderscore t\ worker}\hlopt{;\ );}\\
\hllin{\ 2\ }\hlstd{}\hlkwb{void\ }\hlstd{MyVisionAlgorithm}\hlopt{::}\hlstd{}\hlkwd{doProcessing}\hlstd{}\hlopt{(\ }\hlstd{CVD}\hlopt{::}\hlstd{Image}\hlopt{\usebox{\hlboxlessthan}}\hlstd{CVD}\hlopt{::}\hlstd{byte}\hlopt{\usebox{\hlboxgreaterthan}\&\ }\hlstd{bw}\hlopt{,\ }
\hlstd{\ \ \ \ }\hlstd{\ \ \ \ }\hlstd{\ \ \ \ }\hlstd{CVD}\hlopt{::}\hlstd{Image}\hlopt{\usebox{\hlboxlessthan}\ }\hlstd{CVD}\hlopt{::}\hlstd{Rgb}\hlopt{\usebox{\hlboxlessthan}}\hlstd{CVD}\hlopt{::}\hlstd{byte}\hlopt{\usebox{\hlboxgreaterthan}\ \usebox{\hlboxgreaterthan}\&\ }\hlstd{rgb\ }\hlopt{)\ \usebox{\hlboxopenbrace}}\\
\hllin{\ 3\ }\hlstd{}\hlstd{\ \ \ \ }\hlstd{}\hlslc{//\ My\ code\ goes\ in\ here}\\
\hllin{\ 4\ }\hlstd{}\hlopt{\usebox{\hlboxclosebrace}}\hlstd{}\\
\mbox{}
\normalfont
\normalsize
...
\noindent
\ttfamily
\hlstd{}\hllin{\ 1\ }\hlkwd{MK\textunderscore TOOL\textunderscore PLUGIN}\hlstd{}\hlopt{(\ }\hlstd{my\textunderscore tool}\hlopt{,\ }\hlstd{}\hlstr{"Space"}\hlstd{}\hlopt{,\ }\hlstd{}\hlstr{"Plane"}\hlstd{}\hlopt{,\ );}\\
\hllin{\ 2\ }\hlstd{}\hlkwb{void\ }\hlstd{my\textunderscore tool}\hlopt{::}\hlstd{}\hlkwd{click}\hlstd{}\hlopt{()\ \usebox{\hlboxopenbrace}}\\
\hllin{\ 3\ }\hlstd{}\hlstd{\ \ \ \ }\hlstd{commandList}\hlopt{::}\hlstd{}\hlkwd{exec}\hlstd{}\hlopt{(\ }\hlstd{}\hlcom{/{*}\ Some\ GV3\ command\ here\ {*}/}\hlstd{\ }\hlopt{);}\\
\hllin{\ 4\ }\hlstd{}\hlopt{\usebox{\hlboxclosebrace}}\hlstd{}\\
\mbox{}
\normalfont
\normalsize
...
\noindent
\ttfamily
\hlstd{}\hllin{\ 1\ }\hlkwd{MK\textunderscore GUI\textunderscore COMMAND}\hlstd{}\hlopt{(\ }\hlstd{myCommand}\hlopt{,\ }\hlstd{invokeMyMethod}\hlopt{,\ );}\\
\hllin{\ 2\ }\hlstd{}\hlkwb{void\ }\hlstd{myCommand}\hlopt{::}\hlstd{}\hlkwd{invokeMyMethod}\hlstd{}\hlopt{(\ }\hlstd{string\ params\ }\hlopt{)\ \usebox{\hlboxopenbrace}}\\
\hllin{\ 3\ }\hlstd{}\hlstd{\ \ \ \ }\hlstd{}\hlslc{//\ My\ code\ goes\ in\ here}\\
\hllin{\ 4\ }\hlstd{}\hlopt{\usebox{\hlboxclosebrace}}\hlstd{}\\
\mbox{}
\normalfont
...
\noindent
\ttfamily
\hlstd{}\hlkwc{call\ tree}\hlstd{\ \ \ \ \ \ \ \ \ \ \ \ \ \ \ }\hlkwc{hits}\hlstd{\ \ \ \ \ \ \ }\hlkwc{self\ time}\hlstd{\ \ \ \ \ \ }\hlkwc{total\ time}\hspace*{\fill}\\
\hlstd{\usebox{\hlboxlessthan}}\hlkwc{root}\hlstd{\usebox{\hlboxgreaterthan}}\hlstd{\ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ }\hlstd{}\hlnum{1.0}\hlstd{\ \ \ \ \ \ }\hlnum{8\ }\hlstd{}\hlstd{us}\hlstd{\ \ \ \ }\hlkwc{}\hlstd{}\hlnum{0}\hlstd{\%}\hlstd{\ \ \ \ \ }\hlstd{}\hlnum{54\ }\hlstd{}\hlstd{ms}\hlstd{\ \ }\hlkwc{}\hlstd{}\hlnum{100}\hlstd{\%\hspace*{\fill}\\
}\hlstd{\ \ }\hlstd{}\hlkwc{cam}\hlstd{\ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ }\hlkwc{}\hlstd{}\hlnum{1.0}\hlstd{\ \ \ \ \ }\hlnum{29\ }\hlstd{}\hlstd{ms}\hlstd{\ \ \ }\hlkwc{}\hlstd{}\hlnum{53}\hlstd{\%}\hlstd{\ \ \ \ \ }\hlstd{}\hlnum{29\ }\hlstd{}\hlstd{ms}\hlstd{\ \ \ }\hlkwc{}\hlstd{}\hlnum{53}\hlstd{\%\hspace*{\fill}\\
}\hlstd{\ \ }\hlstd{}\hlkwc{ptam}\hlstd{\ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ }\hlkwc{}\hlstd{}\hlnum{1.0}\hlstd{\ \ \ \ \ }\hlnum{13\ }\hlstd{}\hlstd{ms}\hlstd{\ \ \ }\hlkwc{}\hlstd{}\hlnum{24}\hlstd{\%}\hlstd{\ \ \ \ \ }\hlstd{}\hlnum{13\ }\hlstd{}\hlstd{ms}\hlstd{\ \ \ }\hlkwc{}\hlstd{}\hlnum{24}\hlstd{\%\hspace*{\fill}\\
}\hlstd{\ \ }\hlstd{}\hlkwc{gui}\hlstd{\ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ }\hlkwc{}\hlstd{}\hlnum{1.0}\hlstd{\ \ \ \ \ \ }\hlnum{5\ }\hlstd{}\hlstd{ms}\hlstd{\ \ \ \ }\hlkwc{}\hlstd{}\hlnum{9}\hlstd{\%}\hlstd{\ \ \ \ \ }\hlstd{}\hlnum{13\ }\hlstd{}\hlstd{ms}\hlstd{\ \ \ }\hlkwc{}\hlstd{}\hlnum{23}\hlstd{\%\hspace*{\fill}\\
}\hlstd{\ \ \ \ }\hlstd{}\hlkwc{DrawStuff}\hlstd{\ \ \ \ \ \ \ \ \ \ \ \ }\hlkwc{}\hlstd{}\hlnum{1.0}\hlstd{\ \ \ \ \ }\hlnum{15\ }\hlstd{}\hlstd{us}\hlstd{\ \ \ \ }\hlkwc{}\hlstd{}\hlnum{0}\hlstd{\%}\hlstd{\ \ \ \ \ \ }\hlstd{}\hlnum{8\ }\hlstd{}\hlstd{ms}\hlstd{\ \ \ }\hlkwc{}\hlstd{}\hlnum{14}\hlstd{\%\hspace*{\fill}\\
}\hlstd{\ \ \ \ \ \ }\hlstd{}\hlkwc{DrawPoints}\hlstd{\ \ \ \ \ \ \ \ \ }\hlkwc{}\hlstd{}\hlnum{1.0}\hlstd{\ \ \ \ \ \ }\hlnum{7\ }\hlstd{}\hlstd{ms}\hlstd{\ \ \ }\hlkwc{}\hlstd{}\hlnum{12}\hlstd{\%}\hlstd{\ \ \ \ \ \ }\hlstd{}\hlnum{7\ }\hlstd{}\hlstd{ms}\hlstd{\ \ \ }\hlkwc{}\hlstd{}\hlnum{12}\hlstd{\%\hspace*{\fill}\\
}\hlstd{\ \ \ \ \ \ }\hlstd{}\hlkwc{DrawFeatures}\hlstd{\ \ \ \ \ \ \ }\hlkwc{}\hlstd{}\hlnum{1.0}\hlstd{\ \ \ \ \ }\hlnum{20\ }\hlstd{}\hlstd{us}\hlstd{\ \ \ \ }\hlkwc{}\hlstd{}\hlnum{0}\hlstd{\%}\hlstd{\ \ \ \ \ }\hlstd{}\hlnum{20\ }\hlstd{}\hlstd{us}\hlstd{\ \ \ \ }\hlkwc{}\hlstd{}\hlnum{0}\hlstd{\%\hspace*{\fill}\\
}\hlstd{\ \ \ \ \ \ }\hlstd{}\hlkwc{DrawTarget}\hlstd{\ \ \ \ \ \ \ \ \ }\hlkwc{}\hlstd{}\hlnum{1.0}\hlstd{\ \ \ \ }\hlnum{842\ }\hlstd{}\hlstd{us}\hlstd{\ \ \ \ }\hlkwc{}\hlstd{}\hlnum{2}\hlstd{\%}\hlstd{\ \ \ \ }\hlstd{}\hlnum{842\ }\hlstd{}\hlstd{us}\hlstd{\ \ \ \ }\hlkwc{}\hlstd{}\hlnum{2}\hlstd{\%\hspace*{\fill}\\
}\hlstd{\ \ \ \ \ \ }\hlstd{}\hlkwc{DrawPolys}\hlstd{\ \ \ \ \ \ \ \ \ \ }\hlkwc{}\hlstd{}\hlnum{1.0}\hlstd{\ \ \ \ \ }\hlnum{65\ }\hlstd{}\hlstd{us}\hlstd{\ \ \ \ }\hlkwc{}\hlstd{}\hlnum{0}\hlstd{\%}\hlstd{\ \ \ \ }\hlstd{}\hlnum{200\ }\hlstd{}\hlstd{us}\hlstd{\ \ \ \ }\hlkwc{}\hlstd{}\hlnum{0}\hlstd{\%\hspace*{\fill}\\
}\hlstd{\ \ \ \ \ \ \ \ }\hlstd{}\hlkwc{textureTidyup}\hlstd{\ \ \ \ }\hlkwc{}\hlstd{}\hlnum{9.0}\hlstd{\ \ \ \ \ }\hlnum{19\ }\hlstd{}\hlstd{us}\hlstd{\ \ \ \ }\hlkwc{}\hlstd{}\hlnum{0}\hlstd{\%}\hlstd{\ \ \ \ \ }\hlstd{}\hlnum{19\ }\hlstd{}\hlstd{us}\hlstd{\ \ \ \ }\hlkwc{}\hlstd{}\hlnum{0}\hlstd{\%\hspace*{\fill}\\
}\hlstd{\ \ \ \ \ \ \ \ }\hlstd{}\hlkwc{retrieveTexture}\hlstd{\ \ }\hlkwc{}\hlstd{}\hlnum{9.0}\hlstd{\ \ \ \ \ }\hlnum{12\ }\hlstd{}\hlstd{us}\hlstd{\ \ \ \ }\hlkwc{}\hlstd{}\hlnum{0}\hlstd{\%}\hlstd{\ \ \ \ \ }\hlstd{}\hlnum{12\ }\hlstd{}\hlstd{us}\hlstd{\ \ \ \ }\hlkwc{}\hlstd{}\hlnum{0}\hlstd{\%\hspace*{\fill}\\
}\hlstd{\ \ \ \ \ \ \ \ }\hlstd{}\hlkwc{drawTriangles}\hlstd{\ \ \ \ }\hlkwc{}\hlstd{}\hlnum{9.0}\hlstd{\ \ \ \ }\hlnum{104\ }\hlstd{}\hlstd{us}\hlstd{\ \ \ \ }\hlkwc{}\hlstd{}\hlnum{0}\hlstd{\%}\hlstd{\ \ \ \ }\hlstd{}\hlnum{104\ }\hlstd{}\hlstd{us}\hlstd{\ \ \ \ }\hlkwc{}\hlstd{}\hlnum{0}\hlstd{\%\hspace*{\fill}\\
}\hlstd{\ \ }\hlstd{}\hlkwc{commands}\hlstd{\ \ \ \ \ \ \ \ \ \ \ \ \ \ \ }\hlkwc{}\hlstd{}\hlnum{1.0}\hlstd{\ \ \ \ \ }\hlnum{19\ }\hlstd{}\hlstd{us}\hlstd{\ \ \ \ }\hlkwc{}\hlstd{}\hlnum{0}\hlstd{\%}\hlstd{\ \ \ \ \ }\hlstd{}\hlnum{19\ }\hlstd{}\hlstd{us}\hlstd{\ \ \ \ }\hlkwc{}\hlstd{}\hlnum{0}\hlstd{\%\hspace*{\fill}\\
}\hlstd{\ \ }\hlstd{}\hlkwc{textures}\hlstd{\ \ \ \ \ \ \ \ \ \ \ \ \ \ \ }\hlkwc{}\hlstd{}\hlnum{1.0}\hlstd{\ \ \ \ \ }\hlnum{34\ }\hlstd{}\hlstd{us}\hlstd{\ \ \ \ }\hlkwc{}\hlstd{}\hlnum{0}\hlstd{\%}\hlstd{\ \ \ \ \ }\hlstd{}\hlnum{34\ }\hlstd{}\hlstd{us}\hlstd{\ \ \ \ }\hlkwc{}\hlstd{}\hlnum{0}\hlstd{\%}\hspace*{\fill}\\
\mbox{}
\normalfont
\normalsize
...
In the second pass, the code examines adjacent pieces ot texture; if the frames are taken from sufficiently similar camera angles, it will choose whichever  image gives the most coverage - if the currently examined face is bounded on at least two sides by the same texture, the angle is within the tolerance, and the texture frame can cover the current face, then the replacement will be performed. The results of this clean are illustrated in Figure~\ref{texclean}}
...
\noindent
\ttfamily
\hlstd{}\hlslc{\#\ Material\ database\ for\ scanner\textunderscore model.obj}\hspace*{\fill}\\
\hlstd{}\hlslc{\#}\hlstd{\ \ }\hlslc{Declares\ a\ new\ material\ called\ `TextureMapped'}\hspace*{\fill}\\
\hlstd{}\hlslc{\#}\hlstd{\ \ \ \ }\hlslc{containing\ the\ link\ to\ the\ TGA\ texture\ file,}\hspace*{\fill}\\
\hlstd{}\hlslc{\#}\hlstd{\ \ \ \ }\hlslc{and\ details\ of\ the\ material\ properties}\hspace*{\fill}\\
\hlstd{\hspace*{\fill}\\
newmtl\ TextureMapped\hspace*{\fill}\\
}\hlstd{\ \ \ \ }\hlstd{Ka\ }\hlnum{1.000\ 1.000\ 1.000}\hlstd{\ \ }\hlnum{}\hlstd{}\hlslc{\#\ Ambient\ reflection}\hspace*{\fill}\\
\hlstd{}\hlstd{\ \ \ \ }\hlstd{Kd\ }\hlnum{1.000\ 1.000\ 1.000}\hlstd{\ \ }\hlnum{}\hlstd{}\hlslc{\#\ Diffuse\ reflection}\hspace*{\fill}\\
\hlstd{}\hlstd{\ \ \ \ }\hlstd{Ks\ }\hlnum{0.000\ 0.000\ 0.000}\hlstd{\ \ }\hlnum{}\hlstd{}\hlslc{\#\ Specular\ reflection}\hspace*{\fill}\\
\hlstd{\ \hspace*{\fill}\\
}\hlstd{\ \ \ \ }\hlstd{d\ }\hlnum{1.0}\hlstd{\ \ \ \ }\hlnum{}\hlstd{}\hlslc{\#\ Alpha\ Transparency}\hspace*{\fill}\\
\hlstd{}\hlstd{\ \ \ \ }\hlstd{Tr\ }\hlnum{1.0}\hlstd{\ \ \ }\hlnum{}\hlstd{}\hlslc{\#}\hlstd{\ \ }\hlslc{(some\ implementations\ use\ d,\ others\ Tr)}\hspace*{\fill}\\
\hlstd{}\hlstd{\ \ \ \ }\hlstd{illum\ }\hlnum{2}\hlstd{\ \ }\hlnum{}\hlstd{}\hlslc{\#\ Illumination\ model\ {-}\ Diffuse\ and\ Specular}\hspace*{\fill}\\
\hlstd{}\hlstd{\ \ \ \ }\hlstd{map\textunderscore Ka\ scanner\textunderscore model.tga}\hlstd{\ \ }\hlstd{}\hlslc{\#\ Ambient\ map}\hspace*{\fill}\\
\hlstd{}\hlstd{\ \ \ \ }\hlstd{map\textunderscore Kd\ scanner\textunderscore model.tga}\hlstd{\ \ }\hlstd{}\hlslc{\#\ Diffuse\ map}\hspace*{\fill}\\
\hlstd{}\hlstd{\ \ \ \ \ }\hlstd{}\hlslc{\#\ Currently\ does\ not\ use:}\hspace*{\fill}\\
\hlstd{}\hlstd{\ \ \ \ \ }\hlstd{}\hlslc{\#\ map\textunderscore Ks}\hlstd{\ \ \ }\hlslc{{-}}\hlstd{\ \ }\hlslc{Specular\ map}\hspace*{\fill}\\
\hlstd{}\hlstd{\ \ \ \ \ }\hlstd{}\hlslc{\#\ map\textunderscore d}\hlstd{\ \ \ \ }\hlslc{{-}}\hlstd{\ \ }\hlslc{Alpha\ transparency\ map}\hspace*{\fill}\\
\hlstd{}\hlstd{\ \ \ \ \ }\hlstd{}\hlslc{\#\ map\textunderscore bump\ {-}}\hlstd{\ \ }\hlslc{Bump\ map}\hlstd{}\hspace*{\fill}\\
\mbox{}
\normalfont
\normalsize
...
\hlstd{}\hlslc{\#\ Header\ indicating\ how\ many\ vertices\ and\ faces\ are\ encoded}\hspace*{\fill}\\
\hlstd{}\hlslc{\#}\hlstd{\ \ \ }\hlslc{in\ the\ file,\ for\ human\ interest}\hspace*{\fill}\\
\hlstd{}\hlslc{\#}\hspace*{\fill}\\
\hlstd{}\hspace*{\fill}\\
\hlslc{\#\ Material\ library\ links:}\hspace*{\fill}\\
\hlstd{}\hlstd{\ \ }\hlstd{mtllib\ scanner\textunderscore model.mtl}\hspace*{\fill}\\
\hspace*{\fill}\\
\hlslc{\#\ Vertices:}\hspace*{\fill}\\
\hlstd{}\hlstd{\ \ }\hlstd{v\ }\hlnum{0.112184\ }\hlstd{}\hlopt{{-}}\hlstd{}\hlnum{0.549372\ 0.636787}\hspace*{\fill}\\
\hlstd{}\hlstd{\ \ }\hlstd{v\ }\hlnum{0.130041\ }\hlstd{}\hlopt{{-}}\hlstd{}\hlnum{0.377297\ 0.638883}\hspace*{\fill}\\
\hlstd{}\hlstd{\ \ }\hlstd{v\ }\hlnum{0.246977\ }\hlstd{}\hlopt{{-}}\hlstd{}\hlnum{0.535198\ 0.444692}\hspace*{\fill}\\
\hlstd{}\hlslc{\#}\hlstd{\ \ }\hlslc{etc...}\hspace*{\fill}\\
\hlstd{}\hspace*{\fill}\\
\hlslc{\#\ Texture\ Co{-}Ordinates:}\hspace*{\fill}\\
\hlstd{}\hlstd{\ \ }\hlstd{vt\ }\hlnum{0.424639\ 0.217299}\hspace*{\fill}\\
\hlstd{}\hlstd{\ \ }\hlstd{vt\ }\hlnum{0.273666\ 0.0712889}\hspace*{\fill}\\
\hlstd{}\hlstd{\ \ }\hlstd{vt\ }\hlnum{0.33988\ 0.0545356}\hspace*{\fill}\\
\hlstd{}\hlslc{\#}\hlstd{\ \ }\hlslc{etc...}\hspace*{\fill}\\
\hlstd{}\hspace*{\fill}\\
\hlslc{\#\ Normals\ (currently\ one\ per\ face):}\hspace*{\fill}\\
\hlstd{}\hlstd{\ \ }\hlstd{vn\ }\hlnum{0.0824474\ }\hlstd{}\hlopt{{-}}\hlstd{}\hlnum{0.892047\ 0.444359}\hspace*{\fill}\\
\hlstd{}\hlstd{\ \ }\hlstd{vn\ }\hlnum{0.200107\ }\hlstd{}\hlopt{{-}}\hlstd{}\hlnum{0.88881\ 0.41228}\hspace*{\fill}\\
\hlstd{}\hlstd{\ \ }\hlstd{vn\ }\hlnum{0.971032\ }\hlstd{}\hlopt{{-}}\hlstd{}\hlnum{0.025272\ }\hlstd{}\hlopt{{-}}\hlstd{}\hlnum{0.237608}\hspace*{\fill}\\
\hlstd{}\hlslc{\#}\hlstd{\ \ }\hlslc{etc...}\hspace*{\fill}\\
\hlstd{\hspace*{\fill}\\
g\ scanner\textunderscore model}\hlstd{\ \ }\hlstd{}\hlslc{\#\ Polygon\ group\ name}\hspace*{\fill}\\
\hlstd{}\hlslc{\#\ Faces:}\hspace*{\fill}\\
\hlstd{}\hlstd{\ \ }\hlstd{usemtl\ TextureMapped}\hlstd{\ \ \ }\hlstd{}\hlslc{\#\ Load\ this\ material\ from\ MTL\ file}\hspace*{\fill}\\
\hlstd{}\hlstd{\ \ }\hlstd{f\ }\hlnum{9}\hlstd{}\hlopt{/}\hlstd{}\hlnum{1}\hlstd{}\hlopt{/}\hlstd{}\hlnum{1\ 35}\hlstd{}\hlopt{/}\hlstd{}\hlnum{2}\hlstd{}\hlopt{/}\hlstd{}\hlnum{1\ 33}\hlstd{}\hlopt{/}\hlstd{}\hlnum{3}\hlstd{}\hlopt{/}\hlstd{}\hlnum{1}\hlstd{\ \ }\hlnum{}\hlstd{}\hlslc{\#\ One\ triple\ per\ vertex\ in\ the\ face}\hspace*{\fill}\\
\hlstd{}\hlstd{\ \ }\hlstd{f\ }\hlnum{9}\hlstd{}\hlopt{/}\hlstd{}\hlnum{4}\hlstd{}\hlopt{/}\hlstd{}\hlnum{2\ 39}\hlstd{}\hlopt{/}\hlstd{}\hlnum{6}\hlstd{}\hlopt{/}\hlstd{}\hlnum{2\ 35}\hlstd{}\hlopt{/}\hlstd{}\hlnum{5}\hlstd{}\hlopt{/}\hlstd{}\hlnum{2}\hlstd{\ \ }\hlnum{}\hlstd{}\hlslc{\#\ Each\ triple\ indicates\ vertex\ id,}\hspace*{\fill}\\
\hlstd{}\hlstd{\ \ }\hlstd{f\ }\hlnum{9}\hlstd{}\hlopt{/}\hlstd{}\hlnum{7}\hlstd{}\hlopt{/}\hlstd{}\hlnum{3\ 19}\hlstd{}\hlopt{/}\hlstd{}\hlnum{9}\hlstd{}\hlopt{/}\hlstd{}\hlnum{3\ 35}\hlstd{}\hlopt{/}\hlstd{}\hlnum{8}\hlstd{}\hlopt{/}\hlstd{}\hlnum{3}\hlstd{\ \ }\hlnum{}\hlstd{}\hlslc{\#}\hlstd{\ \ \ }\hlslc{texture\ coord\ id,\ and\ normal\ id}\hspace*{\fill}\\
\hlstd{}\hlslc{\#}\hlstd{\ \ }\hlslc{etc...}\hlstd{}\hspace*{\fill}\\
\mbox{}
\normalfont
\normalsize
...
Throughout the implementation, it has been noted that GVars3 variables are used to expose user-configurable parameters. A full list of these can be seen in Appendix~\ref{configparams}. However, for a user to modify them they really need to know what they're looking for; they could type \texttt{gvarlist} in the tool's terminal, but this will list all variables in both PTAM and this system, many of which a user need not concern themselves with. Furthermore, there is no structure to these values, or indication as to how they should be set. Finally, a user may make mistakes when typing a variable, and not realise their mistake until it is too late. Providing a well-organised graphical interface to the configuration readily resolves many of these issues, while maintaining the user-friendly and un-cluttered nature of the software.
...
However, many of these AR experiences are based on techniques popularised by University of Washington ARToolkit \cite**{artk}, which was itself based on earlier work \cite**{artkpaper} in collaboration between the University of Washington and Hiroshima City University. 
 This so-called \textit{marker based AR} requires tokens (Figure~\ref{markerar}) to extract co-ordinate axes and orient the augmented reality world. These are subject to a number of failure modes, including occlusion or a reduction in image quality due to, for example, motion blur (Figure~\ref{markerfail}). As PTAM tracks the scene based on a much larger number of feature points, if just one collection of points is obscured or untrackable it does not break the entire track.
...
\begin{figure}[t]
    \subfigure[Desired Geometry]{\includegraphics[width=165px]{TriEmpty}}
    \quad
    \subfigure[Incorrectly added \texttt{PolyFace}]{\includegraphics[width=165px]{TriFilled}}
...
\begin{figure}
  \begin{center}
    ...
    \hspace{10px}
    \subfigure[Error Map Intensity Scale]{\includegraphics[width=160px]{ErrorMapScale}\label{errorscale1}}
  \end{center}
...
\begin{figure}
  \begin{center}
    ...
    \hspace{10px}
    \subfigure[Error Map Intensity Scale]{\includegraphics[width=160px]{ErrorMapScale}\label{errorscale2}}
  \end{center}
...
While the percentage error is easy for a person to digest, the error map gives much more detailed information. Ideally, a measure somewhere between the two should provide an easy to compare metric, that provides more detailed information than just a percentage error. In order to do this, Figure~\ref{histograms} shows histograms generated indicating the frequency of each amount of error in the error maps; simply a case of counting the number of occurrences of each pixel intensity in the grayscale map. These make it easy to see that the errors present with a blank model are all present below a peak of 9\%, while those in the modelled case exhibit errors up to 74\% - although under 0.03\% of the image exhibits errors over 70\%. By way of contrast, approximately 40\% of the frame exhibits an error of under 10\%.
...
\item{Option to switch between a fully-automated mode and the manual modelling mode discussed already}
...
\ttfamily
\hlstd{}\hlslc{//}\hspace*{\fill}\\
\hlstd{}\hlslc{//\ Modelling\ Settings}\hspace*{\fill}\\
\hlstd{}\hlslc{//}\hspace*{\fill}\\
\hlstd{}\hspace*{\fill}\\
\hlslc{//\ ARDriver:}\hspace*{\fill}\\
\hlstd{drawBackground\ }\hlopt{=\ }\hlstd{}\hlkwa{true\ }\hspace*{\fill}\\
\hlstd{drawGrid\ }\hlopt{=\ }\hlstd{}\hlkwa{true\ }\hspace*{\fill}\\
\hlstd{}\hspace*{\fill}\\
\hlslc{//\ ARPointRenderer:}\hspace*{\fill}\\
\hlstd{drawPoints\ }\hlopt{=\ }\hlstd{}\hlkwa{true\ }\hspace*{\fill}\\
\hlstd{drawFeatures\ }\hlopt{=\ }\hlstd{}\hlkwa{true\ }\hspace*{\fill}\\
\hlstd{drawModel\ }\hlopt{=\ }\hlstd{}\hlkwa{true\ }\hspace*{\fill}\\
\hlstd{drawTarget\ }\hlopt{=\ }\hlstd{}\hlkwa{true\ }\hspace*{\fill}\\
\hlstd{ptSize\ }\hlopt{=\ }\hlstd{}\hlnum{0.05\ }\hspace*{\fill}\\
\hlstd{drawClosestPoint\ }\hlopt{=\ }\hlstd{}\hlkwa{true\ }\hspace*{\fill}\\
\hlstd{ftSize\ }\hlopt{=\ }\hlstd{}\hlnum{0.01\ }\hspace*{\fill}\\
\hlstd{ftRadius\ }\hlopt{=\ }\hlstd{}\hlnum{1.0}\hspace*{\fill}\\
\hlstd{drawEdges\ }\hlopt{=\ }\hlstd{}\hlkwa{true\ }\hspace*{\fill}\\
\hlstd{drawClosestEdge\ }\hlopt{=\ }\hlstd{}\hlkwa{true\ }\hspace*{\fill}\\
\hlstd{drawFaces\ }\hlopt{=\ }\hlstd{}\hlkwa{true\ }\hspace*{\fill}\\
\hlstd{drawNormals\ }\hlopt{=\ }\hlstd{}\hlkwa{false\ }\hspace*{\fill}\\
\hlstd{drawClosestFace\ }\hlopt{=\ }\hlstd{}\hlkwa{true\ }\hspace*{\fill}\\
\hlstd{drawClosestPlane\ }\hlopt{=\ }\hlstd{}\hlkwa{true\ }\hspace*{\fill}\\
\hlstd{}\hspace*{\fill}\\
\hlslc{//\ Commands:}\hspace*{\fill}\\
\hlstd{ftRadius\ }\hlopt{=\ }\hlstd{}\hlnum{1.0\ }\hspace*{\fill}\\
\hlstd{edgeTolerance\ }\hlopt{=\ }\hlstd{}\hlnum{0.5\ }\hspace*{\fill}\\
\hlstd{edgeTolerance\ }\hlopt{=\ }\hlstd{}\hlnum{0.5\ }\hspace*{\fill}\\
\hlstd{planeTolerance\ }\hlopt{=\ }\hlstd{}\hlnum{0.1\ }\hspace*{\fill}\\
\hlstd{revolveFaces\ }\hlopt{=\ }\hlstd{}\hlnum{10\ }\hspace*{\fill}\\
\hlstd{planeTolerance\ }\hlopt{=\ }\hlstd{}\hlnum{0.1}\hspace*{\fill}\\
\hlstd{texTolerance1\ }\hlopt{=\ }\hlstd{}\hlnum{1.0\ }\hspace*{\fill}\\
\hlstd{texTolerance2\ }\hlopt{=\ }\hlstd{}\hlnum{3.0\ }\hspace*{\fill}\\
\hlstd{catmullSmooth\ }\hlopt{=\ }\hlstd{}\hlnum{0.2\ }\hspace*{\fill}\\
\hlstd{smoothDeformAttraction\ }\hlopt{=\ }\hlstd{}\hlnum{0.8\ }\hspace*{\fill}\\
\hlstd{smoothDeformInertia\ }\hlopt{=\ }\hlstd{}\hlnum{0.5\ }\hspace*{\fill}\\
\hlstd{edgeProb\ }\hlopt{=\ }\hlstd{}\hlnum{0.5\ }\hspace*{\fill}\\
\hlstd{}\hspace*{\fill}\\
\hlslc{//\ Processors:}\hspace*{\fill}\\
\hlstd{echo\ }\hlopt{=}\hlstd{\ \ }\hlopt{}\hlstd{}\hlkwa{false\ }\hspace*{\fill}\\
\hlstd{texCleanFreq\ }\hlopt{=\ }\hlstd{}\hlnum{1000\ }\hspace*{\fill}\\
\hlstd{blurAccuracy\ }\hlopt{=\ }\hlstd{}\hlkwa{true\ }\hspace*{\fill}\\
\hlstd{}\hspace*{\fill}\\
\hspace*{\fill}\\
...
\noindent
\hlslc{//}\hspace*{\fill}\\
\hlstd{}\hlslc{//\ Game\ Settings}\hspace*{\fill}\\
\hlstd{}\hlslc{//}\hspace*{\fill}\\
\hlstd{}\hspace*{\fill}\\
\hlslc{//\ AIUnit:}\hspace*{\fill}\\
\hlstd{aiMass\ }\hlopt{=\ }\hlstd{}\hlnum{5\ }\hspace*{\fill}\\
\hlstd{aiPatience\ }\hlopt{=\ }\hlstd{}\hlnum{100000\ }\hspace*{\fill}\\
\hlstd{aiSpeed\ }\hlopt{=\ }\hlstd{}\hlnum{20\ }\hspace*{\fill}\\
\hlstd{}\hspace*{\fill}\\
\hlslc{//\ Director:}\hspace*{\fill}\\
\hlstd{game}\hlopt{.}\hlstd{impactScore\ }\hlopt{=\ }\hlstd{}\hlnum{0\ }\hspace*{\fill}\\
\hlstd{game}\hlopt{.}\hlstd{deathScore\ }\hlopt{=\ }\hlstd{}\hlnum{100\ }\hspace*{\fill}\\
\hlstd{game}\hlopt{.}\hlstd{goalScore\ }\hlopt{=\ {-}}\hlstd{}\hlnum{300\ }\hspace*{\fill}\\
\hlstd{spawnTolerance\ }\hlopt{=\ }\hlstd{}\hlnum{0.3\ }\hspace*{\fill}\\
\hlstd{spawnFreq\ }\hlopt{=\ }\hlstd{}\hlnum{3000\ }\hspace*{\fill}\\
\hlstd{spawnProb\ }\hlopt{=\ }\hlstd{}\hlnum{0.0001\ }\hspace*{\fill}\\
\hlstd{}\hspace*{\fill}\\
\hlslc{//\ GameFactory:}\hspace*{\fill}\\
\hlstd{maxGradient\ }\hlopt{=\ }\hlstd{}\hlnum{1.0\ }\hspace*{\fill}\\
\hlstd{physicsScale\ }\hlopt{=\ }\hlstd{}\hlnum{1000\ }\hspace*{\fill}\\
\hlstd{}\hspace*{\fill}\\
\hlslc{//\ GameRenderer:}\hspace*{\fill}\\
\hlstd{drawModel\ }\hlopt{=\ }\hlstd{}\hlkwa{true\ }\hspace*{\fill}\\
\hlstd{drawWaypoints\ }\hlopt{=\ }\hlstd{}\hlkwa{true\ }\hspace*{\fill}\\
\hlstd{drawProjectiles\ }\hlopt{=\ }\hlstd{}\hlkwa{true\ }\hspace*{\fill}\\
\hlstd{drawUnits\ }\hlopt{=\ }\hlstd{}\hlkwa{true\ }\hspace*{\fill}\\
\hlstd{waypointSize\ }\hlopt{=\ }\hlstd{}\hlnum{0.02\ }\hspace*{\fill}\\
\hlstd{}\hspace*{\fill}\\
\hlslc{//\ Hooks:}\hspace*{\fill}\\
\hlstd{physicsStep\ }\hlopt{=\ }\hlstd{}\hlnum{1}\hlstd{/}\hlnum{60}\hspace*{\fill}\\
\hlstd{gravity\ }\hlopt{=\ {-}}\hlstd{}\hlnum{15\ }\hspace*{\fill}\\
\hlstd{pushScale\ }\hlopt{=\ }\hlstd{}\hlnum{80\ }\hspace*{\fill}\\
\hlstd{}\hspace*{\fill}\\
\hlslc{//\ Projectile:}\hspace*{\fill}\\
\hlstd{projectileMass\ }\hlopt{=}\hlstd{\ \ }\hlopt{}\hlstd{}\hlnum{0.4\ }\hlstd{}\hspace*{\fill}\\
\mbox{}
\normalfont
\normalsize
...
\chapter{MEL Script to Invoke Tool for Viewing}
\label{melar}

% Yes, this is generated from source. Don't try and edit too much!
\noindent
\ttfamily
\hlstd{}\hllin{\ 1\ }\hlcom{/{*}{*}}\\
...
\hllin{32\ }\hlstd{}\\
\hllin{33\ }\hlcom{/{*}{*}}\\
\hllin{34\ }\hlcom{\ {*}\ Assume\ the\ tool\ saved\ a\ model\ to\ the\ default}\\
\hllin{35\ }\hlcom{\ {*}\ location,\ delete\ the\ current\ model\ and\ import}\\
\hllin{36\ }\hlcom{\ {*}\ the\ tweaked\ model\ from\ disk}\\
\hllin{37\ }\hlcom{\ {*}/}\hlstd{}\\
\hllin{38\ }\hlkwa{global\ proc\ }\hlstd{loadFromViewer}\hlopt{()\ \usebox{\hlboxopenbrace}}\\
\hllin{39\ }\hlstd{}\hlstd{\ \ \ \ }\hlstd{}\hlkwc{\$file\ }\hlstd{}\hlopt{=\ }\hlstd{}\hlstr{"/path/to/Modeller/scanner\textunderscore model.obj"}\hlstd{}\hlopt{;}\\
\hllin{40\ }\hlstd{}\hlstd{\ \ \ \ }\hlstd{\\
\hllin{41\ }}\hlstd{\ \ \ \ }\hlstd{}\hlslc{//\ Delete\ the\ current\ mesh}\\
\hllin{42\ }\hlstd{}\hlstd{\ \ \ \ }\hlstd{}\hlkwa{for}\hlstd{}\hlopt{(\ }\hlstd{}\hlkwc{\$del\ }\hlstd{}\hlkwa{in\ }\hlstd{}\hlstr{`ls\ {-}l\ {-}type\ "mesh"`}\hlstd{\ }\hlopt{)\ \usebox{\hlboxopenbrace}}\\
\hllin{43\ }\hlstd{}\hlstd{\ \ \ \ \ \ \ \ }\hlstd{}\hlkwa{if\ }\hlstd{}\hlopt{(\ }\hlstd{}\hlkwb{objExists}\hlstd{}\hlopt{(\ }\hlstd{}\hlkwc{\$del\ }\hlstd{}\hlopt{)\ )}\\
\hllin{44\ }\hlstd{}\hlstd{\ \ \ \ \ \ \ \ \ \ \ \ }\hlstd{}\hlkwb{delete\ }\hlstd{}\hlkwc{\$del}\hlstd{}\hlopt{;}\\
\hllin{45\ }\hlstd{}\hlstd{\ \ \ \ }\hlstd{}\hlopt{\usebox{\hlboxclosebrace}}\hlstd{\ \ }\hlopt{}\\
\hllin{46\ }\hlstd{}\hlstd{\ \ \ \ }\hlstd{\\
\hllin{47\ }}\hlstd{\ \ \ \ }\hlstd{}\hlslc{//\ Load\ the\ saved\ OBJ\ file}\\
\hllin{48\ }\hlstd{}\hlstd{\ \ \ \ }\hlstd{}\hlkwb{file\ }\hlstd{}\hlopt{{-}}\hlstd{import\ }\hlopt{{-}}\hlstd{type\ }\hlstr{"OBJ"}\hlstd{\ }\hlkwc{\$file}\hlstd{}\hlopt{;}\\
\hllin{49\ }\hlstd{}\hlopt{\usebox{\hlboxclosebrace}}\hlstd{} \\
\hllin{50\ }\hlstd{}\\
\mbox{}
\normalfont
\normalsize
