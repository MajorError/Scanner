...
\begin{figure}[h]
  \begin{center}
        ...
        \begin{tikzpicture}[scale=0.6]
          ...
          \begin{class}[text width=40px]{ToggleGrid}{-3,-12}
            \inherit{Tool}
          \end{class}
...
Given a camera pose $C$, and that $C_{rotation} = \left[
  \begin{array}{c c c}
    x_1 & x_2 & x_3 \\
    y_1 & y_2 & y_3 \\
    z_1 & z_2 & z_3
...
$ Projection ~=~ \left[ 
  \begin{array}{c}
    \frac{Target_x - C_x}{z_1} \\ 
    \frac{Target_y - C_y}{z_2} \\ 
    \frac{Target_z - C_z}{z_3}
  \end{array}\right]$, and $~~lerp ~=~ \left[ 
  \begin{array}{c}
    (1 - lock_x) \times Target_x \\ 
    (1 - lock_y) \times Target_y \\ 
    (1 - lock_z) \times Target_z
  \end{array}\right]$
\\
  
Therefore, new vertex position at camera pose $C'$ is given by 
\\

$ \left[ 
  \begin{array}{c}
    lock_x \times (C'_x + z'_1 \times Projection_x) + lerp_x \\ 
    lock_y \times (C'_y + z'_2 \times Projection_y) + lerp_y \\ 
    lock_z \times (C'_z + z'_3 \times Projection_z) + lerp_z
  \end{array}\right]$
\\
}
\item{\textbf{edge.connect}:}
\item{\textbf{edge.bisect}:}
\item{\textbf{edge.remove}:}
\item{\textbf{plane.move}:}
\item{\textbf{plane.extrude}:}
\item{\textbf{plane.split}:}
\item{\textbf{plane.revolve}:}
\item{\textbf{text.draw}:}
\item{\textbf{texture.clean}:}
\item{\textbf{texture.clear}:}
\item{\textbf{textureBoundary.blend}:}
\item{\textbf{shrinkwrap.exec}:}
\item{\textbf{mesh.subdivide}:}
\item{\textbf{mesh.smoothDeform}:}
\item{\textbf{mesh.move}:}
\item{\textbf{mesh.scale}:}
