...
As already discussed, \texttt{VisionProcessor} plugins provide the backbone of the main loop. In the main system there are six processors invoked in order, making up this main system loop. Each processor is given references to \texttt{Image}s of both black-and-white bytes and RGB bytes. Those in use in this system are:

\begin{itemize}
\item{\texttt{cam}: Responsible for extracting camera frames from PTAM's \texttt{VideoSource}.}
\item{\texttt{ptam}: Invokes PTAM's tracking on each new frame, and updates the \texttt{Environment} with fresh lists of features, as well as the current camera pose. This separation permits decoupling of the PTAM system from the rest of the software.}
\item{\texttt{guiDispatch}: Dispatches events to the GUI, and invokes the rendering of the scene on each iteration.}
\item{\texttt{commandList}: Provides a simple interface, \texttt{static void commandList:: exec( string );} to permit threads across the system to update the \texttt{Environment} in a thread-safe manner, by enqueing GVars3 commands to be executed in the main thread, thereby trivially preventing many race conditions in the code.}
\item{\texttt{textureExtractor}: One of the few completely automated processes in the system is that of texture extraction. This processor compares the current camera frame to the existing frame in use for each \texttt{PolyFace} in the system, and tests whether it would provide a better texture resolution.}
\item{\texttt{profiler}: The system includes the Shiny Profiler\cite{shiny} for debugging purposes; this processor causes Shiny to aggregate data for the past iteration, and output it to the console. This provides detailed timing information for each of the commands and methods that have been invoked in the course of the past iteration.}
\end{itemize}
