...
\begin{figure}
  \begin{center}
%    \includegraphics[width=340px]{TODO}
  \end{center}
...
When it is clear, as it seems to be here, that a novel approach such as this can \textit{help} rather than \textit{hinder} an artist's creativity, it becomes easy to argue that it is an approach worthy of refinement, along with further study.
...
\begin{verbatim}
string $toDelete[];
for( $obj in `ls -l -type "mesh"` ) {
    string $tri[] = `polyTriangulate -ch 1 $obj`;
    $toDelete[size( $toDelete )] = $tri[0];
}

file -op "groups=1;ptgroups=1;materials=1;smoothing=1;normals=1;" 
        -typ "OBJexport" -pr -ea "/path/to/tool/tmp.obj";
...
// TODO: Invoke tool with file as param
\end{verbatim}
