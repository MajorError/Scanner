...
\subsection{Overview}
Having discussed the background, and broad description of the system to be created, the following section gives a detailed and precise set of both functional- and non-functional requirements for the system to follow.

\subsection{Requirements}
\begin{itemize}
\item{User input is to be limited to a maximum of two keys; one to select their ``tool" of choice, one to invoke it}
\item{All other input is to be taken from a single video camera - minimal requirement here is support for the UniBrain Fire-i, Video4Linux support is to be considered optional but desirable}
\item{Unix platform support is required, at minimum; it is considered a boon if the software also works on recent Windows platforms}
\item{The User Interface should present a single window, with as few visual controls as possible}
\item{The UI window should present a live video feed from the camera}
\item{Relevant markers may be shown on this video feed to indicate highlighted features}
\item{Further overlay must include the current state of the user's modelling efforts at all times}
\item{It is important that the user can still see their target object on the screen; it may be desirable to be able to toggle the display of the model, or at minimum to render it alpha-blended with the original scene}
\item{The frame-rate of this overlay window should not drop below an average of 20fps during processing; it is important that the user experiences a real-time workflow}
\item{The system will perform continuous localisation and tracking using the PTAM framework}
\item{Tools in the system must operate on triangular polygons in the model; either to create, delete, or manipulate them}
\item{It is vital that tools exist to correct any mistakes the user may make before they continue; the workflow should be non-linear, and up to the user's choice as to which order they use the tools}
\item{Tools may operate at the complete model, polygon face, edge, or vertex level}
\item{The system must extract texture data from the visual scene to continuously enhance its model; it should use the best available historical texture for each face of the model}
\item{It is acceptable to use tools in the system to perform enhanced processing on the model; for example, to perform texture homogenisation across faces}
\item{The system must be capable of exporting 3D model data, texture data, and UV maps to a common standard for import into a 3D modelling package}
\item{There should be at least one file format that the system is capable of importing back into a new scene; issues of scale will need to be dealt with here}
\item{The system should provide a command-line scripting style interface so that the model may be manipulated manually by a more technical user, or potentially scripted using shell redirection}
\item{This scripting interface should permit all aspects of the model to be manipulated in a consistent manner; be this through vertex identifiers, or another scheme}
\item{Configuration and scripting commands should be handled by the GVars3 interface, as used by LibCVD and PTAM, which will already be in use in the system, in order to reduce dependency creep}
\item{Any computer vision augmentation, scene processors, etc must be optional; they must either offer a means for the user to disable them, or an alternate version which does not use vision-based techniques}
\item{It must be possible for the user to model a given object from all angles, regardless of the shape of the object (i.e. concave as well as convex hulls)}
\item{The system's memory footprint should be proportional to the complexity of the model; it is vital that it does not ``leak" memory over time, as complex modelling may take a considerable period of time}
\item{The architecture of the final product must be carefully planned, such that individual components may be swapped out in favour of new approaches at any time; to this end, some form of extensibility is required}
\end{itemize}

\subsection{Documentation}
As well as source code for a working product fitting all of the above requirements, a number of documentation requirements must be met. As one might expect, some form of on- or off-line guide for using the product would be invaluable in helping others to learn how to use it. This should cover (at minimum) the basics of modelling, how to correct mistakes, and troubleshooting for when things go wrong. It should also cover technique for camera calibration, and how to get the most out of the vision-augmented toolset (a user who is not familiar with vision algorithms may find this non-intuitive).

The second, and altogether more important form of documentation is a complete report covering the problem, the rationale for the approach, and details of the implementation. This is deliverable as \LaTeX~source, along with full cross-referencing to a bibtex reference file. When describing the problem, it is vital to consider the current state of the art, and justify the approach taken. This should make extensive reference to relevant publications. The solution developed should show that it has assimilated this collective background experience, and enhanced the results shown in these papers using the approach described previously. It is anticipated that this section be based on the Background and Approach outlined in this document. The main focus of the report, however, should to be on the details of the implementation. This should include architectural rationale (with diagrams, where applicable) as well as details of the vision algorithms used. It should focus on the core implementation of the product, and then on the enhancements to the user experience created by application of vision techniques.

The methodology for evaluation is discussed in the next section. However, the report should include a critical evaluation of the software against its goals. Ideally, the numerical techniques described below should be used to quantify and evaluate the various vision-based enhancements the solution develops to include. Finally, the report should be concluded with a critical discussion of the success of the final product, when measured against the influences and prior work discussed in the background section, as well as suggestions on future work and research approaches that could enhance the work already completed.
