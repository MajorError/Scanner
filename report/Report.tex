...
\node[state, name=view, below of=user] {View};
\node[state, name=controller, below of=view, right of=view, yshift=-4em, xshift=4em] {Controller};
\node[state, name=model, left of=controller, xshift=-16em] {Model};

\draw ($(view.north) + (-.5em,0)$)
edge[stateEdge] node[edgeLabel, xshift=3em]{\emph{interacts}}
($(user.south) + (-.5em,0)$);

\draw ($(user.south) + (.5em,0)$)
edge[stateEdge] node[edgeLabel, xshift=-3em]{\emph{informs}}
($(view.north) + (.5em,0)$);

\draw ($(view.east) + (0,0)$)
edge[stateEdge, bend left=30] node[edgeLabel, xshift=2em, yshift=1em]{requests}
($(controller.north) + (0,0)$);

\draw ($(controller.west) + (0,0)$)
edge[stateEdge] node[edgeLabel, xshift=0, yshift=-1em]{manipulates}
($(model.east) + (0,0)$);

\draw ($(view.west) + (0,0)$)
edge[stateEdge, bend right=30] node[edgeLabel, xshift=-2em, yshift=1em]{adapts}
($(model.north) + (0,0)$);
%
\end{tikzpicture} 
